...
var PulsarEslintFix = {
  ...
  activate: function () {
    ...
    atom.workspace.observeTextEditors((editor) => {
      editor.onDidSave(() => {
        ...
        if (!this.running) {
          if (this.config.notifications.get()) {
            this.logNotification("", "Running auto-prettier-eslint");
          }
          this.running = true;
          this.run();
        }
  ...
  cliExec: function (cwd, runner, arg, callback) {
    this.child = execFile(
      ...
      (error, stdout, stderr) => {
        ...
        args.detail = err ? err + "\n" + out : out;
        if (this.config.notifications.get()) {
          this.logNotification("", runner + " compleated");
        }
        if (runner.includes("eslint")) {
          this.current_error = notif.contents? args.detail : "";
        }
        this.running = false;
        this.child = null;
        callback();
  ...
  provideLinter: function () {
    return {
      ...
      lint: (editor) => {
        let promises = [];
        let parsing = this.current_error;
        while (parsing.length > 0 && parsing.indexOf("\n") !== 0) {
          ...
          promises.push({
            ...
            location: {
              file: path,
              position: helpers.generateRange(editor, Number(line) - 1, Number(column) -1),
            }
          });
        }
